...
myApp.config(['$stateProvider',function($stateProvider){
	$stateProvider
		...
		.state('employeeParent',{   //home是状态的名字，不是路由名字
			url: '/employee',   //路由的名字
			templateUrl: './src/scripts/ui-router/employeeParent.html',
			controller: 'employeeParentController'
		})
		.state('employeeParent.employee',{   //home是状态的名字，不是路由名字
			url: '/',   //路由的名字
		...
		.state('employeeParent.employeeDetail',{   //home是状态的名字，不是路由名字
			url: '/:id&:firstName',   //路由的名字
			templateUrl: './src/scripts/ui-router/employeedetail.html',
			controller: 'employeeDetailController',
			controllerAs: 'employeeDetailCtrl'
		})
		.state('employeeParent.employeeSearch',{   //home是状态的名字，不是路由名字
			url: '/:keywords',   //路由的名字
...
myApp.controller('homeController',['$state',function($state){
	this.message = "Home Page";

	// console.log($state.current.data.customData);
	this.customData1 = $state.current.data.customData;
	this.customData2 = $state.get('course').data.customData; //跨路由获取数据，ngRoute实现不了
}])

myApp.controller('courseController',['$scope','$state',function($scope,$state){
